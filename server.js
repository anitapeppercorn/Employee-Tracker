...
// List of packages used and dependencies
...
// MySQL DB Connection Information user specific credentials)
...
//starting function of Employee Tracker application
...
// render logo image
function renderLogoImage() { const longText =
'We are creating an interface that makes it easy ' +
'for non-developers to view and interact with ' +
...
//Using Inquirer package in the same manner as Readme Generator project as a UI
function runSearch(){
   inquirer
       .prompt({
           type: "list",
           ...
           choices: ["View All Employees", "View All Employees by Department", "View All Employees by Manager", "View Roles", "View Departments", "Add Employee", "Add Roles", "Add Departments", "Delete Employee", "Delete Role", "Delete Department", "Update Employee Role", "Update Employee Manager", "View Total Utilized Budget By Department", chalk.red("Exit Program")]
         })
       .then(answer => {
...
function promptDepartments(departments){
   inquirer
       ...
       .then(answer => {
           viewItEmployeesByDepartment(answer.promptChoice);           
       });
}
...
function promptManagers(managers){
   inquirer
       ...
       .then(answer => {
           viewItEmployeesByManager(answer.promptChoice);           
       });
}
 
//Choose to see all the employees
function viewItEmployeesAll(){
   const query = `
   ...
   connection.query(query, (err, res) => {
       if (err) throw err;
       //build our table array from choices
       const tableData = [];
...
//Choose to see all departments
function viewItDepartments(){
   const query = `SELECT department.name FROM department;`;
   connection.query(query, (err, res) => {
       if (err) throw err;
       //gather department names in an array
       const departments = [];
       for (let i = 0; i < res.length; i++) {
           departments.push(res[i].name);
       }
       //Choose to see a specific department
       promptDepartments(departments)
   });
}
 
function viewItDepartmentsCallBack(callback){
   const query = `SELECT department.name FROM department;`;
   connection.query(query, (err, res) => {
       if (err) throw err;
       //gather the department name
       const departments = [];
       for (let i = 0; i < res.length; i++) {
           departments.push(res[i].name);
       }
       //Choose to select a department
...
// Gather departments w/o employees
function viewItDepartmentsOnly(){
...
//Gather Roles only and display
function viewItRolesOnly() {
...
//look at all managers
function viewItManagers(){
   const query = `
...
//look at employees by department
function viewItEmployeesByDepartment(department){
   //sql query
   const query = `
...
//Choose to see the employees by their manager
function viewItEmployeesByManager(manager){
   //sql query
   const query = `
   SELECT employee.id, employee.first_name, employee.last_name, role.title, role.salary, department.name AS department_name, concat(manager.first_name, " ", manager.last_name) AS manager_full_name
   FROM employee
   ...
   connection.query(query, (err, res) => {
       if (err) throw err;
       //Generate information to show from the selected result
       const tableData = [];
       for (let i = 0; i < res.length; i++) {
           tableData.push({
               "ID": res[i].id,
...
function addEmployee(){
   ...
   //Use inquirer to prompt for the name of a new employee
   inquirer
       .prompt([{
           name: "firstName",
       ...
       .then(answers => {
           ...
           connection.query(query, (err, res) => {
               ...
               //Use inquirer to prompt for role selection
               inquirer
               .prompt({
                   type: "list",
               ...
               .then(answer => {
                   ...
                   connection.query(query, (err, res) => {
                       ...
                       //Use inquirer to prompt for the name of manager
                       inquirer
                       .prompt({
                           type: "list",
                       ...
                       .then(answer => {
                           ...
                           connection.query(query, {
                               ...
                               setTimeout(viewItEmployeesAll, 500);
                           });                           
                       });
                   });
               });
...
function addDepartment() {
   //Use inquirer to prompt for the name of Department
   inquirer
     .prompt([
       {
         name: "dName",
     ...
     .then((answer) => {
       ...
       connection.query(query, [answer.dName], (err, res) => {
           ...
           viewItDepartmentsCallBack(function(departments) {
...
function addRole() {
   ...
   //Use inquirer to prompt for the title
   inquirer
       .prompt([
           {
           name: "rName",
   ...
   connection.query(query, (err, res) => {
       ...
       .then(answer => {
            ...
            connection.query(query, {id: chosenRoleID}, (err, res) => {
               ...
               setTimeout(viewItRolesOnly, 500);
           });
       });
   });
}
   ...
   connection.query(query, (err, res) => {
       ...
       .then(answer => {
            ...
            connection.query(query, {id: chosenDepartmentId}, (err, res) => {
               ...
               setTimeout(viewItDepartmentsOnly, 500);   
...
function updateEmployeeManager(){
   //initialize & update Employee object
   const updatedEmployee = {
       id: 0,
       managerID: 0
   };
   ...
   connection.query(query, (err, res) => {
       ...
       //Use inquirer to prompt for the name of employee to update
       inquirer
       .prompt({
           type: "list",
           name: "employeePromptChoice",
